...
RECONNECT_INTERVAL_MS = 2000 # Try to reconnect every 2 seconds
LOG_DIRECTORY = "logs"
...
class SerialMonitor(QWidget):
    ...
    def _setup_ui(self):
        ...
        last_port = config.get('serial_monitor.last_used_port')
        last_baud = str(config.get('serial_monitor.last_used_baudrate', '115200'))
        ...
        control_layout.addWidget(self.auto_reconnect_checkbox)
    ...
    def toggle_connection(self):
        ...
        elif self.is_attempting_reconnect:
            self._stop_current_connection()
        else:
    ...
    def _handle_connection_status(self, is_connected):
        if is_connected:
            ...
            config.settings['serial_monitor']['last_used_port'] = port
            config.settings['serial_monitor']['last_used_baudrate'] = int(baud)
            ...
            if self.auto_reconnect_checkbox.isChecked():
                ...
                self.connect_button.setText("Cancel Reconnect")
    ...
    def _attempt_reconnect(self):
        ...
        if port_to_find in available_ports:
            self._log_to_terminal(f"--- Port {port_to_find} found. Reconnecting... ---", "#00FF7F")
            self.reconnect_timer.stop()
            self._start_new_connection()
    ...
    def _handle_data_received(self, data):
        if self.log_file and not self.log_file.closed:
            ...
            try:
                self.log_file.write(f"[{timestamp}] {data.decode('utf-8', errors='ignore')}")
    ...
    def _update_terminal_view(self):
        ...
        is_at_bottom = self.terminal_view.verticalScrollBar().value() == self.terminal_view.verticalScrollBar().maximum()
    ...
    def _start_logging(self):
        self._stop_logging() # Ensure any previous file is closed
        filename = datetime.now().strftime(f"serial_log_%Y-%m-%d.log")
        filepath = os.path.join(LOG_DIRECTORY, filename)
        try:
            self.log_file = open(filepath, "a", encoding="utf-8")
            self._log_to_terminal(f"--- Logging to {filepath} ---\n\r", "#ADD8E6")
        except Exception as e:
