...
class SerialWorker(QObject):
    ...
    def __init__(self, port, baudrate):
        super().__init__(); self.serial_conn = None; self._port = port; self._baudrate = baudrate; self._is_running = False
        ...
        try:
            ...
            while self._is_running:
                try:
                    ...
                    self.error_occurred.emit(f"Device disconnected: {e}"); break
        except serial.SerialException as e: self.error_occurred.emit(f"Connection failed: {e}")
        finally:
...
class SerialMonitor(QWidget):
    def __init__(self):
        super().__init__()
        self.serial_thread = None; self.serial_worker = None; self.rx_buffer = bytearray(); self.log_file = None
        ...
        self.update_timer = QTimer(self); self.update_timer.setInterval(UPDATE_INTERVAL_MS); self.update_timer.timeout.connect(self._process_rx_buffer)
    ...
    # MODIFICATION: Rewritten to handle live highlighting without breaking autoscroll.
    def _add_text_to_terminal(self, text: str, color: QColor):
        ...
        should_highlight = bool(highlight_term) and \
                           (highlight_term in text if is_case_sensitive else highlight_term.lower() in text.lower())
        
        # --- Critical Section: Text Insertion ---
        ...
        # --- Brute-force scrollbar control (original logic) ---
        if auto_scroll_enabled:
            scroll_bar.setValue(scroll_bar.maximum())
        else:
            scroll_bar.setValue(old_scroll_value)
        
        # --- Prune old text (original logic) ---
        document = self.terminal_view.document()
        if document.blockCount() > MAX_TERMINAL_BLOCKS:
    ...
    def _start_new_connection(self, port, baud):
        self.user_initiated_disconnect = False; self.serial_thread = QThread(); self.serial_worker = SerialWorker(port, baud); self.serial_worker.moveToThread(self.serial_thread)
        self.serial_worker.data_received.connect(self.on_data_received); self.serial_worker.error_occurred.connect(lambda msg: self._add_text_to_terminal(f"\n--- {msg} ---\n", COLOR_ERROR))
        self.serial_worker.connection_status.connect(self.on_connection_status_changed); self.serial_thread.started.connect(self.serial_worker.run)
        self.serial_thread.start(); self.connect_button.setText("Connecting..."); self.port_combo.setEnabled(False); self.baud_combo.setEnabled(False)

    def _stop_current_connection(self):
        self.user_initiated_disconnect = True; self.reconnect_timer.stop()
        if self.serial_worker: self.serial_worker.stop()
        if self.serial_thread: self.serial_thread.quit(); self.serial_thread.wait(500); self.serial_thread = None; self.serial_worker = None

    @Slot(bool)
    def on_connection_status_changed(self, is_connected):
        self.is_attempting_reconnect = getattr(self, 'is_attempting_reconnect', False)
        if is_connected:
            self.is_attempting_reconnect = False; self.reconnect_timer.stop(); self.connect_button.setChecked(True); self.connect_button.setText("Disconnect")
            self._add_text_to_terminal(f"\n--- Connected to {self.port_combo.currentText()} @ {self.baud_combo.currentText()} baud ---\n", COLOR_INFO)
            config.settings['serial_monitor']['last_used_port'] = self.port_combo.currentText(); config.settings['serial_monitor']['last_used_baudrate'] = int(self.baud_combo.currentText())
            self.start_file_logging(); self.update_timer.start()
        else:
            self.port_combo.setEnabled(True); self.baud_combo.setEnabled(True); self.connect_button.setChecked(False); self.connect_button.setText("Connect")
            self.stop_file_logging(); self.update_timer.stop(); self._process_rx_buffer()
            if self.auto_reconnect_checkbox.isChecked() and not getattr(self, 'user_initiated_disconnect', False):
                if not self.is_attempting_reconnect: self._add_text_to_terminal("\n--- Connection lost. Attempting to reconnect... ---\n", COLOR_WARN); self.is_attempting_reconnect = True; self.reconnect_timer.start()
            else: self._add_text_to_terminal("\n--- Disconnected ---\n", COLOR_WARN)
    ...
    def _process_rx_buffer(self):
        ...
        decoded = self.rx_buffer.decode('utf-8', 'ignore'); processed_len = 0
        for line in decoded.splitlines(keepends=True):
            if line.endswith(('\n', '\r')):
                self._add_text_to_terminal(line, COLOR_RX); processed_len += len(line.encode('utf-8', 'ignore'))
            else: break
    ...
    def _attempt_reconnect(self):
        if self.port_combo.currentText() in [p.device for p in serial.tools.list_ports.comports()]: self._add_text_to_terminal("\n--- Port found. Reconnecting... ---\n", COLOR_INFO); self.reconnect_timer.stop(); self.is_attempting_reconnect = False; self.toggle_connection(True)
